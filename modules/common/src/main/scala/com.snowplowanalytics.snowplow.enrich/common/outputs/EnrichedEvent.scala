...
class EnrichedEvent extends Serializable {
  ...
  def toKv(k: String, v: String): Seq[(String, Json)] = if (v != null) Seq((k, Json.fromString(v))) else Seq.empty[(String, Json)]
  def toKv(k: String, v: JByte): Seq[(String, Json)] = if (v != null) Seq((k, Json.fromBoolean(v == 1))) else Seq.empty[(String, Json)]
  def toKv(k: String, v: JInteger): Seq[(String, Json)] = if (v != null) Seq((k, Json.fromInt(v))) else Seq.empty[(String, Json)]
  def toKv(k: String, v: JFloat): Seq[(String, Json)] = if (v != null) Seq((k, Json.fromFloatOrNull(v))) else Seq.empty[(String, Json)]
  def toDateKv(k: String, v: String): Seq[(String, Json)] =
    if (v != null)
      Seq(
        (k,
         Json.fromString(
           DateTimeFormatter.ISO_DATE_TIME.format(JsonSchemaDateTimeFormat.parse(v))
         )
        )
      )
